...
seed records.
The manufacturing overview, connector registry, agent registry and action
registry API reference surfaces now read tenant-scoped `demo_reference_records`
bootstrap rows and return explicit API errors when those records are missing or
invalid. Remaining API-owned reference records are tracked for migration to
